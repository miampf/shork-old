...
use std::{
    fmt::Display,
    ops::{Add, BitAnd, BitOr, Div, Mul, Neg, Not, Rem, Shl, Shr, Sub},
};
            ...
            2 => {
                if self.v_f.is_some() && other.get_float().is_some() {
                    return Some(
                        self.v_f
                            .unwrap()
                            .partial_cmp(&other.get_float().unwrap())
                            .unwrap(),
                    );
                }
                None
                ...
                if self.v_s.is_some() && other.get_string().is_some() {
                    return Some(
                        self.v_s
                            .clone()
                            .unwrap()
                            .cmp(&other.get_string().clone().unwrap()),
                    );
                }
                None
            }
        ...
        match self.r_type {
            ...
            3 => {
                if self.v_s.is_some() && rhs.get_string().is_some() {
                    return Self::string(
                        self.v_s.unwrap() + rhs.get_string().clone().unwrap().as_str(),
                    );
                }
        ...
        match self.r_type {
            0 => {
                res = match rhs.get_type() {
                    ...
                    _ => Self::error(format!(
                        "Unsupported Operation '&' on boolean and {}",
                        rhs.get_type_string()
                    )),
                }
            }
            1 => {
                res = match rhs.get_type() {
                    ...
                    _ => Self::error(format!(
                        "Unsupported Operation '&' on integer and {}",
                        rhs.get_type_string()
                    )),
                }
            }
            _ => {
                res = Self::error(format!(
                    "Unsupported Operation '&' on {} and {}",
                    self.get_type_string(),
                    rhs.get_type_string()
                ))
...
impl ExprEvaluator {
        ...
        if self.match_t(r.val(), vec![EqualEqual, ExclamationEqual]) {
            ...
            res = match r.val().token_type() {
                ...
                _ => {
                    ...
                    let e = ShorkError::generate_error(
                        ErrorType::InterpreterError,
                        r.val().position(),
                        self.source.clone(),
                        format!("Expected '==' or '!=', found {:?}", r.val().token_type()),
                    );
                    Err(e)
                }
            }
    ...
    fn primary(&mut self, tree: &mut AST, n: usize) -> Result<ShorkExprEvalResult, ShorkError> {
        ...
        let mut res = match r.val().token_type() {
            ...
            StringType => Ok(ShorkExprEvalResult::string(
                r.val().content_string().unwrap(),
            )),
            RegexType => Ok(ShorkExprEvalResult::string(
                r.val().content_string().unwrap(),
            )),
            _ => {
                let e = ShorkError::generate_error(
                    ...
                    self.source.clone(),
                    format!(
                        "Unexpected symbol {:?}! Did you mean something else?",
                        r.val().token_type()
                    ),
                );
                Err(e)
