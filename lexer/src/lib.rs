...
mod tests {
    ...
    fn lexing_keywords() {
        ...
        assert_eq!(
            v,
            vec![
                Integer, Float, Char, String, Boolean, Regex, T, Reef, Get, From, As, Define, And,
                Or, For, While, Do, In, If, Else, Return, Structure, Implement, Private, Eof
            ]
        )
    }
    ...
    fn lexing_vars() {
        let source =
            "test = \"Hello\" test2 = 44 test_3 = 12.2 Test4 = false test5uwu = #iamregex#";
        let mut r = StderrReporter::new();
        let mut l = Lexer::new(source.to_string(), &mut r);
        let res = l.scan_tokens();
        if res.is_err() {
