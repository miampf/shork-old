use crate::tree::{Node, AST};
use shork_error::{report::Reporter, ErrorType, ShorkError};
use shork_lexer::tokens::{
    Token,
    TokenType::{self, *},
};
...
impl<'a> ExprParser<'a> {
    ...
    fn comparison(&mut self) -> AST {
        ...
        while self.match_token(vec![Greater, GreaterEqual, Lesser, LesserEqual]) {
            ...
            self.check_error(
                vec![LeftParen, Exclamation, Minus, IntegerType, FloatType],
                format!("Expected number, found {:?}", self.peek().token_type()),
            );
    ...
    fn unary(&mut self) -> AST {
        ...
        if self.match_token(vec![Exclamation, Minus]) {
            ...
            if self.previous().token_type() == &Exclamation {
                self.check_error(
                    vec![BooleanType, Exclamation, LeftParen],
                    format!(
                        "Expected a boolean, '!' or '(', found {:?}",
            ...
            if self.previous().token_type() == &Minus {
                self.check_error(
                    vec![IntegerType, FloatType, Minus],
                    format!("Expected a number, found {:?}", self.peek().token_type()),
                );
            }
